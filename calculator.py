...
class ContractCalculator:
        ...
        *,
        ...
        for it in base_items:
            ...
            for price, lot_float, wear in lots:
                ...
                if self._bid_mode:
                    _is_st = bool(it.get('is_stattrak')) or (
                        'stattrak' in str(name).lower() and it.get('is_stattrak') is not False
                    )
                    bid_p = self.price_manager.get_bid_price(
                        str(name), target_wear=str(wear) if wear else None,
    ...
    def _optimize_contract_wear_distribution(
        ...
        *,
        ...
        result = []
        for i, (price, norm, skin_name, wear, max_float_for_wear) in enumerate(assigned):
            s2 = dict(contract_skins[i])
            s2['name'] = skin_name
            s2['wear'] = wear
            s2['price'] = price
            s2['float'] = max_float_for_wear
