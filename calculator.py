...
class ContractCalculator(_PriceLookupMixin):
    ...
    def __init__(self, database: CS2Database, price_manager: PriceManager):
        ...
        self._memo_collection_output_range: Dict[Tuple, Optional[Tuple[float, float]]] = {}
                    ...
                    for filler_c in filler_choices:
                        ...
                        for base_target2 in base_targets:
                            ...
                            if expected_wear is None:
                                expected_wear = self._determine_wear_from_float(float(current_eval.get('average_float') or current_eval.get('average_normalized_float') or 0.0))
            ...
            for target_c in top_targets:
                ...
                for target_cnt, filler_cnt in splits:
                    ...
                    for filler_c in filler_collection_choices:
                        ...
                        avg_norm_val = float(current_eval.get('average_float') or current_eval.get('average_normalized_float') or 0.0)
    ...
    def _get_collection_output_float_range(self, collection: str, input_rarity: str, is_stattrak: bool) -> Optional[Tuple[float, float]]:
        ...
        memo_key = (collection, input_rarity, next_rarity, bool(is_stattrak))
        with self._lock_next_grade:
            cached = self._memo_collection_output_range.get(memo_key)
        if cached is not None:
            return cached

        outputs = self._get_possible_outputs(collection, input_rarity, average_float=None, is_stattrak=is_stattrak)
        if not outputs:
            with self._lock_next_grade:
                self._memo_collection_output_range[memo_key] = None
            return None

        min_f = None
        max_f = None
        for out in outputs:
            skin_data = self.database.get_skin_by_name(str(out.get('name') or ''))
            if not skin_data:
                continue
            try:
                ...
                continue
            if min_f is None or smin < min_f:
                min_f = smin
            if max_f is None or smax > max_f:
                max_f = smax
        ...
        result = (float(min_f), float(max_f))
        with self._lock_next_grade:
            self._memo_collection_output_range[memo_key] = result
        return result
    ...
    def _calculate_weighted_average_normalized_float(self, contract_skins: List[Dict], is_stattrak: bool) -> float:
        ...
        if total_weight <= 0:
            self._logger.warning(f"_calculate_weighted_average_normalized_float: all {len(contract_skins)} skins have no valid float data")
            return 0.0
    ...
    def calculate_contract_outcomes_details(self, contract_skins: List[Dict], is_stattrak: bool) -> List[Dict]:
        ...
        # out_float = avg_normalized_float * (max_float - min_float) + min_float
        # Используем нормализованный float (каждый входной скин нормализуется
        # относительно своего min/max диапазона), а не простое среднее абсолютных float.
        avg_float = self._calculate_average_normalized_float(contract_skins)
        if avg_float < 0.0:
            avg_float = 0.0
        if avg_float > 1.0:
            avg_float = 1.0
    ...
    def _calculate_contract_profit(self, contract_skins: List[Dict], target_collection: str,
        ...
        # Расчет среднего флоута входа (нормализованный)
        avg_float = self._calculate_average_normalized_float(contract_skins)
